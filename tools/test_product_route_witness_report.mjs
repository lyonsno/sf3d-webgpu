...
function validInput(over = {}) {
  return {
    ...
    cooperativeReports: {
      'dinov2-tokenizer': { status: 'succeeded', schedulingMode: 'cooperative', issuedGpuDutyCount: 24, retiredGpuDutyCount: 24, inFlightGpuDutyCount: 0 },
      'two-stream-backbone': { status: 'succeeded', schedulingMode: 'cooperative', issuedGpuDutyCount: 300, retiredGpuDutyCount: 300, inFlightGpuDutyCount: 0 },
      'post-processor': { status: 'succeeded', schedulingMode: 'cooperative', completionPolicy: 'bounded-prefix', issuedGpuDutyCount: 702, retiredGpuDutyCount: 702, inFlightGpuDutyCount: 0, maxObservedInFlightGpuDuties: 2 },
      'texture-bake': { status: 'succeeded', schedulingMode: 'cooperative', issuedGpuDutyCount: 61, retiredGpuDutyCount: 61, inFlightGpuDutyCount: 0 },
    },
...
const falsifiers = [
  ...
  ['unsettled cooperative report', validInput({ cooperativeReports: { ...validInput().cooperativeReports, 'post-processor': { status: 'succeeded', schedulingMode: 'cooperative', issuedGpuDutyCount: 702, retiredGpuDutyCount: 700, inFlightGpuDutyCount: 2 } } }), /post-processor left 2 GPU duties in flight/],
  ['disabled scheduling reported as cooperative', validInput({ cooperativeReports: { ...validInput().cooperativeReports, 'dinov2-tokenizer': { status: 'succeeded', schedulingMode: 'disabled', issuedGpuDutyCount: 24, retiredGpuDutyCount: 24, inFlightGpuDutyCount: 0 } } }), /dinov2-tokenizer scheduling mode disabled != cooperative/],
