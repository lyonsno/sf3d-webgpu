 ...
 *
 * Runs cranial's runWebGpuCooperativeAdapterConformance() (kit REQUIRED_KIT_VERSION) over the
 ...
 *   - the kit runtime version is exactly REQUIRED_KIT_VERSION;
...
// --- 1. Kit is exactly REQUIRED_KIT_VERSION -------------------------------------------------
...
assert.equal(typeof kit.runWebGpuCooperativeAdapterConformance, 'function',
  'kit must export runWebGpuCooperativeAdapterConformance');
...
// --- 4. Full adapter conformance: all four scenarios pass --------------------
{
  ...
  assert.equal(report.kitVersion, REQUIRED_KIT_VERSION, `report kitVersion must be ${REQUIRED_KIT_VERSION}`);
