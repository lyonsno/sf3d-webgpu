  ...
  const result = await page.evaluate(async (BATCH) => {
    ...
    async function arm(opts) {
      ...
      return {
        ...
        cooperative: coop ? { schedulingMode: coop.schedulingMode, status: coop.status, queueCompletionAuthority: coop.queueCompletionAuthority, rangeCount: coop.boundaries?.[0]?.actualRangeCount ?? null, completed: coop.progress?.completedItems, total: coop.progress?.totalItems } : null,
      };
    }
